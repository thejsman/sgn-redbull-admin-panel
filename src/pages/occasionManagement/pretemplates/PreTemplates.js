...
const PreTemplates = () => {
  ...
  const breadcrumb = [{ link: "", linkText: "Predefined Templates" }];
  ...
  const editPages = (occasionName, templateName) => {
    history.push("/predefined-templates/edit/" + occasionName + "/" + templateName);
  };
  ...
  const handleDeleteTemplate = () => {
    ...
    deleteOccasionTemplate(params).then((res) => {
      ...
      if (status === 200) {
        ...
        getTemplateList(occasionName.replace("pre_",""));
      } else {
  ...
  return (
    <div className="page_wrapper">
      <Modal show={confirmModal} onHide={handleClose} centered>
        ...
        <Modal.Body>
          <p className="text-center">
            Are you sure you want to delete this Predefined Template?
          </p>
      ...
      <div className="twocol sb page_header">
        <h2>Predefined Template Management</h2>
        <Link to="/predefined-templates/create" className="btn btn-primary btn-sm">
          Add Template
        </Link>
      </div>
      <div className="twocol sb page_header">
            ...
            <tbody>
              {tempalateList.length ? (
                tempalateList?.map((item, i) => (
                  <tr key={i}>
                    <td>
                      <span className="">{item.occasionName.replace("pre_", "")}</span>
                    </td>
                    <td>
