...
const weeksDays = ['', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thrusday', 'Friday', 'Saturday']
...
export const modifyDashBoardData = (apiResult) => {
    let usersData = [], usersLables = [], videosData = [], videosLables = [], viewsData = [], viewsLabel = [];
...
export const checkUrlType=(url)=>{
    let urlRegex = /(https?:\/\/|www\.)[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&\/=]*)/ig;
   if(url) {
       if (url.match(urlRegex))
           return url;
       else
           return siteSetting.api.s3_url + url;
   } else
